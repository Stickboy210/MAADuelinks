...
# 初始化主UI窗口
def init_main_ui():
    global root, current_task_label, toggle_state, running, tasker
    ...
    # 存储选择框的当前值
    global selected_world, selected_portal_level, selected_activity_type, selected_portal_key, manual_portal_input_var, manual_entry_var, use_duelist_beads_var, cycle_time_entry, activity_reverse_var, key_vars, clan_copy_var
    ...
    # 在主界面中添加下拉框
    global maze_floor_var
    maze_floor_var = tk.StringVar(root)
    maze_floor_var.set(maze_floor_options[0])  # 默认选择第一层

    # 创建输入框对应的变量
    global manual_entry_var, use_duelist_beads_var, cycle_time_entry, manual_portal_entry, activity_reverse_var, key_vars, clan_copy_var
    ...
    def open_url(event):
        import webbrowser
        webbrowser.open("https://space.bilibili.com/3546639744633147")
...
def run_start_pipeline(tasker,TaskList):
    ...
    while TaskList:
        ...
        try:
            if(TaskNum[0] == 2):
                if len(TaskNum) > 2 and TaskNum[2]:  # 检查 TaskNum[2] 是否存在且不为空
                    cycle_time = int(TaskNum[2])  # 获取循环执行时间（分钟）
                    cycle_time_seconds = cycle_time * 60  # 转换为秒
                    cycle_time_milliseconds = cycle_time_seconds * 1000  # 转换为毫秒
                    ...
                    def repeat_task():
                        ...
                        if running:
                            ...
                            root.after(cycle_time_milliseconds, repeat_task)  # 递归调用，非阻塞式等待
                    ...
                    #     tasker.post_pipeline("NewHomePage")  # 回到主页
            elif(TaskNum[0] == 1):
                if(len(TaskNum)>1):
...
device = None  # 全局变量，用于存储选中的设备


def main():
    global tasker, selected_device
    
    # 1. 显示设备选择窗口
    selected_device = show_adb_devices()
    if not selected_device:
        print("No device selected. Exiting.")
        return
    
    print(f"Selected device: {selected_device.address}")
    ...
    print("MAA initialized successfully.")
...
def show_adb_devices():
    ...
    # 创建一个列表框来显示设备
    listbox = tk.Listbox(window, height=10, width=150)
    for device_address in adb_devices:
        listbox.insert(tk.END, device_address)
    listbox.pack(pady=20)

    # 确认按钮
    def on_select():
        global device  # 使用 nonlocal 关键字来修改外部的 device 变量
        selection = listbox.curselection()
        if selection:
            device = adb_devices[selection[0]]
            messagebox.showinfo("Selected Device", f"Selected device: {device}")
            window.destroy()  # 关闭窗口
        else:
            messagebox.showwarning("No Selection", "Please select a device.")

    confirm_button = tk.Button(window, text="Select", command=on_select)
    confirm_button.pack(pady=10)

    # 阻塞直到窗口关闭
    window.mainloop()

    return device  # 返回用户选择的设备
